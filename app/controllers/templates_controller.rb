class TemplatesController < ApplicationController
  ...
  # GET /companies.json
  def index
    redirect_to root_path unless Permissions::Template.new(current_user).show?
  ...
  def new
    redirect_to root_path unless Permissions::Template.new(current_user).create?
    @template = Template.new
  end

  def edit
    redirect_to root_path unless Permissions::Template.new(current_user).edit?
  end
  ...
  def destroy
    redirect_to root_path unless Permissions::Template.new(current_user).delete?
    @template.destroy
    respond_to do |format|
